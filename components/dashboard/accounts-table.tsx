...
export function AccountsTable({ accounts, searchQuery }: AccountsTableProps) {
  const [deleteConfirm, setDeleteConfirm] = useState<string | null>(null)
  const [editAccount, setEditAccount] = useState<Account | null>(null)
  const { deleteAccount } = useStore()
  const { toast } = useToast()

  const filteredAccounts = accounts.filter((account) => account.title.toLowerCase().includes(searchQuery.toLowerCase()))
